...
function shopDomain(context: PlatformApiClientContext) {
  return String(context.safeMetadata?.shop_domain || context.storeUrl || "").replace(/^https?:\/\//, "").replace(/\/$/, "");
}
...
function mockDetails(context: PlatformApiClientContext) {
  return {
    ...
    platform: "SHOPIFY",
    shopDomain: shopDomain(context),
    credentialType: context.credentialType,
    readOnly: true
  };
}

export function createShopifyPlatformClient(): PlatformApiClient {
  return {
    async getPlatformIdentity(context) {
      return {
        status: PlatformHealthCheckStatus.HEALTHY,
        message: "Shopify store identity is ready for read-only checks.",
    ...
    async checkAuthentication(context) {
      return {
        ...
        message: context.credentialSecret ? "Shopify credential is available to the internal read client." : "Shopify credential is missing.",
        safeDetails: mockDetails(context),
    ...
    async checkReadPermissions(context) {
      return {
        status: PlatformHealthCheckStatus.HEALTHY,
        message: "Shopify order read permission check is simulated and ready.",
    ...
    async checkWebhookCapability(context) {
      return {
        ...
        message: "Shopify webhook registration remains a later foundation phase.",
    ...
    async checkFulfillmentOrTrackingCapability(context) {
      return {
        status: PlatformHealthCheckStatus.HEALTHY,
        message: "Shopify fulfillment read capability is ready for future simulation.",
        safeDetails: { ...mockDetails(context), capability: "fulfillment_read" }
      };
    }
  };
}
