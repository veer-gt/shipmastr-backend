...
function makeClient() {
  ...
  function matchesIntent(intent: any, where: any): boolean {
    if (!where) return true;
    return Object.entries(where).every(([key, condition]) => {
      if (key === "OR") return (condition as any[]).some((child: any) => matchesIntent(intent, child));
  ...
  const client: RtoNdrRecoveryDb = {
    ...
    rtoNdrRecoveryIntent: {
      ...
      findFirst: async (input: any = {}) => state.intents.find((intent) => matchesIntent(intent, input.where)) ?? null
    },
    growthOffer: {
      create: async ({ data }) => {
        const offer = {
...
async function makeApp(client: RtoNdrRecoveryDb) {
  const app = express();
...
describe("RTO/NDR recovery routes", () => {
  ...
  it("creates, lists, activates, resolves, creates, and reads intents through growth-network routes", async () => {
    ...
    const created = await request(app.baseUrl, "/growth-network/rto-ndr-recovery/policies", {
      method: "POST",
      body: JSON.stringify({
        merchant_id: "merchant_1",
        title: "Confirm failed delivery address",
        action_type: "CONFIRM_ADDRESS",
        incentive_amount_paise: 5000,
        allowed_failure_reasons: ["unreachable"],
        metadata: {
          buyerEmail: "buyer@example.com",
          providerRef: "provider_secret",
          safe: true
        }
      })
    });
    ...
    const listed = await request(app.baseUrl, "/growth-network/rto-ndr-recovery/policies?merchant_id=merchant_1");
    assert.equal(listed.status, 200);
    assert.equal(listed.body.data.policies.length, 1);

    const active = await request(app.baseUrl, "/growth-network/rto-ndr-recovery/policies/policy_1/status", {
      method: "PATCH",
    ...
    const resolved = await request(app.baseUrl, "/growth-network/rto-ndr-recovery/resolve", {
      method: "POST",
      body: JSON.stringify({
        merchant_id: "merchant_1",
    ...
    assert.equal(resolved.body.data.offer.label, "Address confirmation");
    assert.equal(JSON.stringify(resolved.body).includes("buyer@example.com"), false);
    assert.equal(JSON.stringify(resolved.body).includes("provider_secret"), false);

    const intent = await request(app.baseUrl, "/growth-network/rto-ndr-recovery/intents", {
      method: "POST",
      body: JSON.stringify({
        policy_id: "policy_1",
        merchant_id: "merchant_1",
        ...
        idempotency_key: "recovery-route-1"
      })
    });
    assert.equal(intent.status, 201);
    assert.equal(intent.body.data.intentId, "intent_1");
    assert.equal(intent.body.data.communicationSent, false);
    assert.equal(intent.body.data.courierMutation, false);
    assert.equal(intent.body.data.paymentCollection, false);
    assert.equal(JSON.stringify(intent.body).includes("shipment_1"), false);

    const fetched = await request(app.baseUrl, "/growth-network/rto-ndr-recovery/intents/intent_1");
  ...
  it("rejects invalid policy and intent requests at the route boundary", async () => {
    ...
    const negativeAmount = await request(app.baseUrl, "/growth-network/rto-ndr-recovery/policies", {
      method: "POST",
      body: JSON.stringify({
        merchant_id: "merchant_1",
        title: "Invalid recovery amount",
        action_type: "CONFIRM_ADDRESS",
        incentive_amount_paise: -1
      })
    });
    assert.equal(negativeAmount.status, 400);
    assert.equal(negativeAmount.body.error, "VALIDATION_ERROR");

    const invalidAction = await request(app.baseUrl, "/growth-network/rto-ndr-recovery/policies", {
      method: "POST",
      body: JSON.stringify({
        merchant_id: "merchant_1",
        title: "Invalid recovery action",
        action_type: "CALL_COURIER"
      })
    });
    assert.equal(invalidAction.status, 400);

    const missingResolveContext = await request(app.baseUrl, "/growth-network/rto-ndr-recovery/resolve", {
      method: "POST",
      body: JSON.stringify({
        merchant_id: "merchant_1"
      })
    });
    assert.equal(missingResolveContext.status, 400);

    const missingIntentContext = await request(app.baseUrl, "/growth-network/rto-ndr-recovery/intents", {
      method: "POST",
      body: JSON.stringify({
        policy_id: "policy_1",
        merchant_id: "merchant_1"
