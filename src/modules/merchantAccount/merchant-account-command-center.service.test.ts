import assert from "node:assert/strict";
import { readFileSync } from "node:fs";
import { describe, it } from "node:test";
import { buildMerchantAccountCommandCenter } from "./merchant-account-command-center.service.js";
import { requireMerchantCommandCenterActor } from "./merchant-account.routes.js";

function countFrom(map: Record<string, number>) {
  return async ({ where }: any = {}) => {
    const status = where?.status;
    const key = (Array.isArray(status?.in) ? status.in.join("|") : undefined)
      ?? (typeof status === "string" ? status : undefined)
      ?? where?.imageRetentionStatus
      ?? (where?.OR ? "OR" : "total");
    return map[key] ?? 0;
  };
}

function makeClient(overrides: Record<string, any> = {}) {
  return {
    merchant: {
      findUnique: async ({ where }: any) => {
        assert.equal(where.id, "merchant_1");
        return {
          id: "merchant_1",
          name: "Safe Merchant",
          onboardingStatus: "READY_TO_SHIP",
          pickupAddressStatus: "COMPLETED",
          kycStatus: "COMPLETED",
          bankStatus: "COMPLETED",
          firstShipmentStatus: "COMPLETED",
          sellerKycStatus: "VERIFIED"
        };
      }
    },
    order: {
      count: countFrom({
        total: 10,
        "CREATED|RISK_SCORED|VERIFIED|HELD|NEEDS_ATTENTION": 2,
        READY_TO_SHIP: 3,
        "HELD|NEEDS_ATTENTION|NDR|RTO": 1
      })
    },
    shipment: {
      count: countFrom({
        total: 6,
        "draft|rates_fetched|manifested|pickup_scheduled": 2,
        "picked_up|in_transit|out_for_delivery": 1,
        delivered: 2,
        "rto_initiated|rto_in_transit|rto_delivered|cancelled": 1,
        "delivery_failed|lost|damaged|exception": 1
      })
    },
    pickupLocation: { count: async () => 1 },
    ndrCase: { count: async () => 1 },
    rtoCase: { count: async () => 1 },
    codLedgerEntry: {
      count: countFrom({
        "pending|due|delayed": 2,
        "collected|remitted": 1
      })
    },
    sellerWalletLedger: {
      count: async () => 3,
      findFirst: async () => ({
        balanceAfter: 1200,
        currency: "INR",
        status: "POSTED",
        postedAt: new Date("2026-06-29T09:00:00.000Z"),
        createdAt: new Date("2026-06-29T08:00:00.000Z")
      })
    },
    shippingWeightProof: {
      count: countFrom({
        ACTIVE: 1,
        OR: 1
      })
    },
    customer: { count: async () => 4 },
    ...overrides
  };
}

describe("merchant account command center", () => {
  it("mounts the read-only merchant account route behind JWT auth", () => {
    const routes = readFileSync("src/routes/index.ts", "utf8");
    assert.match(routes, /apiRouter\.use\("\/merchant\/account", requireJwtAuth, merchantAccountRouter\);/);
  });

  it("allows merchant actors and denies seller actors", async () => {
    const merchantClient = {
      user: {
        findUnique: async () => ({
          id: "user_1",
          merchantId: "merchant_1",
          role: "MERCHANT_OWNER",
          userType: "MERCHANT_ACCOUNT",
          merchant: { onboardingStatus: "READY_TO_SHIP" }
        })
      }
    };
    const sellerClient = {
      user: {
        findUnique: async () => ({
          id: "user_2",
          merchantId: "merchant_1",
          role: "SELLER_OWNER",
          userType: "SELLER_ACCOUNT",
          merchant: { onboardingStatus: "IN_PROGRESS" }
        })
      }
    };

    await assert.doesNotReject(() => requireMerchantCommandCenterActor({
      userId: "user_1",
      merchantId: "merchant_1"
    }, merchantClient as any));
    await assert.rejects(
      () => requireMerchantCommandCenterActor({ userId: "user_2", merchantId: "merchant_1" }, sellerClient as any),
      /MERCHANT_ACCOUNT_ONLY/
    );
  });

  it("denies another merchant scope", async () => {
    await assert.rejects(
      () => requireMerchantCommandCenterActor({
        userId: "user_1",
        merchantId: "merchant_2"
      }, {
        user: {
          findUnique: async () => ({
            id: "user_1",
            merchantId: "merchant_1",
            role: "MERCHANT_OWNER",
            userType: "MERCHANT_ACCOUNT",
            merchant: { onboardingStatus: "READY_TO_SHIP" }
          })
        }
      } as any),
      /MERCHANT_SCOPE_DENIED/
    );
  });

  it("builds a merchant-scoped command center without unsafe leaks", async () => {
    const result = await buildMerchantAccountCommandCenter("merchant_1", makeClient() as any);
    const serialized = JSON.stringify(result);

    assert.equal(result.merchantProfile.id, "merchant_1");
    assert.equal(result.orderSummary.total, 10);
    assert.equal(result.shipmentSummary.total, 6);
    assert.equal(result.queues.find((queue: any) => queue.key === "ready-to-ship")?.count, 5);
    assert.equal(result.queues.find((queue: any) => queue.key === "needs-attention")?.count, 2);
    assert.equal(result.weightGuardSummary.proofCaptured, 1);
    assert.equal(result.weightGuardSummary.proofArchivedAfterPayout, 1);
    assert.equal(result.weightGuardSummary.uploadMode, "backend_mediated");
    assert.ok(result.links.orders.startsWith("/merchant/"));
    assert.equal(serialized.includes("/seller/"), false);
    assert.equal(serialized.includes("courierPartner"), false);
    assert.equal(serialized.includes("provider"), false);
    assert.equal(serialized.includes("objectKey"), false);
    assert.equal(serialized.includes("bucket"), false);
    assert.equal(serialized.includes("signedUrl"), false);
    assert.equal(serialized.includes("Bearer"), false);
    assert.equal(serialized.includes("secret"), false);
  });

  it("orders next actions deterministically", async () => {
    const result = await buildMerchantAccountCommandCenter("merchant_1", makeClient({
      sellerWalletLedger: {
        count: async () => 1,
        findFirst: async () => ({
          balanceAfter: 100,
          currency: "INR",
          status: "POSTED",
          postedAt: new Date("2026-06-29T09:00:00.000Z"),
          createdAt: new Date("2026-06-29T08:00:00.000Z")
        })
      },
      shippingWeightProof: {
        count: countFrom({ ACTIVE: 0, OR: 0 })
      }
    }) as any);

    assert.deepEqual(result.nextActions.map((action: any) => action.key).slice(0, 4), [
      "needs-attention",
      "ready-to-ship",
      "ndr-returns",
      "wallet-review"
    ]);
    assert.ok(result.nextActions.some((action: any) => action.key === "weight-guard-proof-missing"));
  });

  it("returns safe empty states when optional modules are missing", async () => {
    const result = await buildMerchantAccountCommandCenter("merchant_1", {
      merchant: makeClient().merchant
    } as any);

    assert.equal(result.orderSummary.total, 0);
    assert.equal(result.shipmentSummary.total, 0);
    assert.equal(result.walletSummary.status, "no_activity");
    assert.equal(result.codSummary.status, "no_activity");
    assert.equal(result.weightGuardSummary.status, "no_activity");
    assert.equal(result.pickupWarehouseSummary.status, "needs_setup");
    assert.ok(result.nextActions.length > 0);
  });

  it("keeps deleted Weight Guard metadata merchant safe", async () => {
    const result = await buildMerchantAccountCommandCenter("merchant_1", makeClient({
      shippingWeightProof: {
        count: countFrom({ ACTIVE: 0, OR: 2 })
      }
    }) as any);

    assert.equal(result.weightGuardSummary.proofCaptured, 0);
    assert.equal(result.weightGuardSummary.proofArchivedAfterPayout, 2);
    assert.equal(result.weightGuardSummary.status, "evidence_available");
    assert.equal("imageObjectKey" in result.weightGuardSummary, false);
  });
});
