...
const schema = z.object({
  ...
  H2B_PUBLIC_PROVIDER_INGRESS_ENABLED: envBoolean(false),
  H2B_SHOPIFY_BODY_LIMIT_BYTES: z.coerce.number().int().min(1024).max(1024 * 1024).default(256 * 1024),
  H2B_WOOCOMMERCE_BODY_LIMIT_BYTES: z.coerce.number().int().min(1024).max(1024 * 1024).default(256 * 1024),
  H2B_MAGENTO_BODY_LIMIT_BYTES: z.coerce.number().int().min(1024).max(1024 * 1024).default(64 * 1024),
  ...
  PLACES_MIN_QUERY_CHARS: z.coerce.number().int().min(1).max(20).default(3),
  PLACES_DEBOUNCE_MS: z.coerce.number().int().min(0).max(5000).default(300),
  ...
  ADDRESS_PHONE_PEPPER: z.string().min(16).optional(),
  CHECKOUT_ADDRESS_SESSION_TOKEN_SECRET: z.string().min(32).optional(),
  CHECKOUT_DEV_OTP_CODE: z.string().regex(/^\d{6}$/).optional(),
  ADDRESS_NETWORK_SHADOW_ENABLED: envBoolean(true),
  ...
  WALLET_W1_ALLOW_CASHOUT: envBoolean(false),
