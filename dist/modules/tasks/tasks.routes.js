...
tasksRouter.post("/email/lead-notification", async (req, res) => {
...
tasksRouter.post("/address-geocode", async (req, res) => {
    const body = addressGeocodeTaskSchema.parse(req.body);
    const result = await processAddressGeocodeTask(body.taskId);
    res.json(result);
});
tasksRouter.post("/notifications", async (_req, res) => {
    res.json({
        ok: true,
        task: "notifications"
    });
});
tasksRouter.post("/ndr-actions", async (_req, res) => {
    res.json({
        ok: true,
        task: "ndr"
    });
