...
webhooksRouter.post("/carrier", async (req, res) => {
    const signatureValid = verifyWebhookSignature(req.rawBody ??
        Buffer.from(JSON.stringify(req.body)), req.header("x-shipmastr-signature") ?? undefined);
    ...
    const order = body.externalOrderId
    ...
    if (order) {
        const mappedStatus = body.eventType === "shipment.delivered" ? "DELIVERED" :
