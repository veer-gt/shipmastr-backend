...
import { createOrderDataSignals, createOrderIntelligenceSnapshot, createShipmentDetailsForOrder } from "../intelligence/order-intelligence.service.js";
...
function stricterCodDecision(left, right) {
    return codDecisionRank[left] >= codDecisionRank[right] ? left : right;
}
function riskReasons(value) {
    return Array.isArray(value) ? value.map((reason) => String(reason)) : [];
}
function codShieldRiskTier(decision) {
    if (decision.codDecision === "PREPAID_ONLY" || decision.shipmentDecision === "DO_NOT_SHIP")
        return "HIGH";
    if (decision.codDecision === "MANUAL_REVIEW" || decision.automationStatus === "INTERNAL_REVIEW")
        return "HIGH";
    if (decision.codDecision === "REQUIRE_OTP" || decision.automationStatus === "OTP_SENT")
        return "MEDIUM";
    if (decision.automationStatus === "ADDRESS_CORRECTION_SENT")
        return "MEDIUM";
    return "LOW";
}
function recommendedCodShieldAction(decision) {
...
function sellerSafeRiskSummary(decision) {
    return [
...
function courierShipmentMatchesOrder(shipment, order) {
    if (!shipment.orderId)
        return false;
...
function findCourierShipmentForOrder(order, shipmentsByOrderKey) {
    const candidates = [
...
function buildCourierShipmentMap(courierShipments) {
    const shipmentsByOrderKey = new Map();
...
function shipmentWeightFromCourierShipment(courierShipment, fallback) {
    if (!courierShipment)
        return fallback;
...
function trackingUrlForAwb(awbNumber) {
    return awbNumber ? `/tracking/?awb=${encodeURIComponent(awbNumber)}` : null;
}
function normalizedStatus(value) {
    return String(value || "").trim().toLowerCase().replace(/[\s-]+/g, "_");
}
function isDeliveredShipmentStatus(value) {
    return normalizedStatus(value) === "delivered";
}
function isCodPaymentMode(value) {
    return String(value || "").trim().toUpperCase() === "COD";
}
function codRemittanceMatchesOrder(remittance, order, awbNumber) {
    if (remittance.merchantId !== order.merchantId)
        return false;
    if (awbNumber && remittance.awb === awbNumber)
        return true;
    if (remittance.orderId === order.id || remittance.orderId === order.externalOrderId)
        return true;
    return remittance.externalOrderId === order.externalOrderId;
}
function findCodRemittanceForOrder(order, awbNumber, codRemittances) {
...
function sellerSettlementMatchesOrder(settlement, order, awbNumber) {
    if (settlement.merchantId !== order.merchantId)
        return false;
    if (awbNumber && settlement.awb === awbNumber)
        return true;
    return settlement.orderId === order.id || settlement.orderId === order.externalOrderId;
}
function findSellerSettlementForOrder(order, awbNumber, sellerSettlements) {
    return sellerSettlements
        .filter((settlement) => sellerSettlementMatchesOrder(settlement, order, awbNumber))
        .sort((left, right) => latestTimestamp(right) - latestTimestamp(left))[0] ?? null;
}
function sellerSettlementMetadataObject(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return {};
    return value;
}
function sellerPayoutReleaseConfirmed(metadata) {
    return metadata.financeReleaseConfirmed === true || metadata.releasedForPayoutProcessing === true;
}
function sellerPayoutSandboxPaid(metadata) {
...
function metadataDateString(metadata, key) {
    const value = metadata[key];
...
export function buildSellerSafeOrders(input) {
    const courierById = input.courierById ?? new Map();
...
export function buildOrderAutomationPayloads(order, decision) {
    const basePayload = {
        orderId: order.id,
        externalOrderId: order.externalOrderId,
        orderValue: order.orderValue,
...
export function buildOrderAutomationEvents(order, decision) {
...
ordersRouter.post("/", requireIdempotency, async (req, res) => {
    ...
    try {
        const result = await prisma.$transaction(async (tx) => {
            ...
            const scoredOrder = { ...order, status: "RISK_SCORED" };
        ...
        void Promise.all(eventsToEmit).catch((error) => prisma.auditLog.create({
            data: {
...
ordersRouter.get("/", async (req, res) => {
    ...
    const orderKeys = Array.from(new Set(orders.flatMap((order) => [order.id, order.externalOrderId]).filter(Boolean)));
    const courierShipments = orderKeys.length
        ...
        : [];
    const courierIds = Array.from(new Set(orders
    ...
    const awbKeys = Array.from(new Set([
        ...
        ...orders.map((order) => order.shipmentDetails?.awb).filter((awb) => Boolean(awb))
    ]));
