...
import { GcsStorefrontAssetStorageAdapter, MAX_STOREFRONT_ASSET_BYTES, STOREFRONT_ASSET_UPLOAD_IF_GENERATION_MATCH, STOREFRONT_ASSET_UPLOAD_IF_GENERATION_MATCH_HEADER, STOREFRONT_ASSET_UPLOAD_LENGTH_RANGE, STOREFRONT_ASSET_UPLOAD_LENGTH_RANGE_HEADER } from "./storefront-asset-storage.js";
const requiredEnv = [
...
function requiredHeaders(contentType) {
    return {
...
function mutateSignedObjectPath(uploadUrl) {
    const url = new URL(uploadUrl);
    url.pathname = url.pathname.replace(/\/[^/]+$/, `/mutated-${randomUUID()}.png`);
    return url.toString();
}
function assertGcsRejected(response, label) {
    assert.ok(response.status === 400 || response.status === 403, `${label} expected GCS rejection status 400/403, got ${response.status}`);
}
...
describe("Tier A storefront GCS signed upload proof", () => {
    tierAIt("enforces signed headers, size, expiry, path, and generation precondition", async () => {
        ...
        const cleanupPaths = [];
        async function signedPath(suffix, expiresAt = new Date(Date.now() + 5 * 60 * 1000)) {
        ...
        try {
            ...
            const wrongTypeResponse = await putObject(wrongType.signed.uploadUrl, Buffer.from("png"), { ...requiredHeaders("image/png"), "content-type": "image/jpeg" });
            ...
            delete omittedRangeHeaders[STOREFRONT_ASSET_UPLOAD_LENGTH_RANGE_HEADER];
            const omittedRangeResponse = await putObject(omittedRange.signed.uploadUrl, Buffer.from("png"), omittedRangeHeaders);
            assertGcsRejected(omittedRangeResponse, "omitted content-length-range header");
            const oversized = await signedPath("oversized");
            const oversizedResponse = await putObject(oversized.signed.uploadUrl, Buffer.alloc(MAX_STOREFRONT_ASSET_BYTES + 1024 * 1024), requiredHeaders("image/png"));
            assert.notEqual(oversizedResponse.status, 200);
            const maxAllowed = await signedPath("max-allowed");
            const maxAllowedResponse = await putObject(maxAllowed.signed.uploadUrl, Buffer.alloc(MAX_STOREFRONT_ASSET_BYTES - 1), requiredHeaders("image/png"));
            assert.equal(maxAllowedResponse.status, 200);
            const expired = await signedPath("expired", new Date(Date.now() + 2 * 1000));
            await new Promise((resolve) => setTimeout(resolve, 4 * 1000));
            const expiredResponse = await putObject(expired.signed.uploadUrl, Buffer.from("png"), requiredHeaders("image/png"));
            assertGcsRejected(expiredResponse, "expired signed URL");
            const mutated = await signedPath("mutated");
            const mutatedResponse = await putObject(mutateSignedObjectPath(mutated.signed.uploadUrl), Buffer.from("png"), requiredHeaders("image/png"));
            assertGcsRejected(mutatedResponse, "mutated signed object path");
            const precondition = await signedPath("precondition");
            const firstPut = await putObject(precondition.signed.uploadUrl, Buffer.from("png"), requiredHeaders("image/png"));
            assert.equal(firstPut.status, 200);
            const secondPut = await putObject(precondition.signed.uploadUrl, Buffer.from("png again"), requiredHeaders("image/png"));
            assert.equal(secondPut.status, 412);
        }
        finally {
            await Promise.all(cleanupPaths.map((gcsPath) => storage.bucket(bucket).file(gcsPath).delete({ ignoreNotFound: true }).catch(() => undefined)));
        }
    });
});
//# sourceMappingURL=storefront-gcs-signed-upload-tier-a.test.js.map