...
function signCourierToken(user) {
    return jwt.sign({
        userId: user.id,
...
function phoneSellerEmail(mobile) {
...
function sellerUserResponse(user, merchant) {
...
function courierUserResponse(user, courier) {
    return {
        id: user.id,
        name: user.name,
        email: user.email,
...
authRouter.post("/register/verify-code", async (req, res) => {
    ...
    const pending = pendingRecord.metadata;
...
function resolvePasswordAccount(req) {
    ...
    try {
        ...
        if (role === UserRole.COURIER) {
            if (!decoded.userId || !decoded.courierId)
...
authRouter.post("/login", async (req, res) => {
    ...
    if (!user)
        throw new HttpError(400, "INVALID_LOGIN");
    if (isProtectedMasterAdminEmail(user.email) &&
